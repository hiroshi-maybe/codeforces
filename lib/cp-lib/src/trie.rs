...
impl TrieNode {
    ...
    pub fn insert(&mut self, id: usize, mut cs: Chars) {
    ...
    pub fn erase(&mut self, id: usize, mut cs: Chars) -> bool {
        if let Some(c) = cs.next() {
            let i = self.child_node_index(&c);
            if let Some(child) = self.children[i].as_mut() {
                child.erase(id, cs)
    ...
    pub fn find(&self, mut cs: Chars) -> Option<&TrieNode> {
        if let Some(c) = cs.next() {
            let i = self.child_node_index(&c);
...
mod tests_trie_tree {
    ...
    fn test_insert_and_erase() {
        ...
        assert_eq!(trie.erase(0, "app".chars()), false);
        assert_eq!(trie.erase(1, "app".chars()), true);
