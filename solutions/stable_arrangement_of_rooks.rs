...
// $ cp-batch stable_arrangement_of_rooks | diff stable_arrangement_of_rooks.out -
// $ cargo run --bin stable_arrangement_of_rooks
...
fn solve() -> Option<Vec<Vec<char>>> {
    ...
    if k > (n + 1) / 2 {
        ...
        let mut res = vvec!('.'; n; n);
        for i in (0..).step_by(2).take(k) {
            res[i][i] = 'R';
        }
        Some(res)
    }
