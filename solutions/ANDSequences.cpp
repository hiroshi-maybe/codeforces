...
// $ cp-batch ANDSequences | diff ANDSequences.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address ANDSequences.cpp && ./a.out
...
void solve() {
  VI cnt(32);
  REP(i,N) REP(j,32) cnt[j]+=ith(A[i],j);
  int x=0;
  REP(i,32) if(cnt[i]==N) x|=(1<<i);
  int edgecnt=0;
  REP(i,N) edgecnt+=A[i]==x;
  auto res=com.perm(edgecnt,2)*com.fact(N-2);
  cout<<res<<endl;
}
