...
// $ cp-batch minor_reduction | diff minor_reduction.out -
// $ cargo run --bin minor_reduction
...
/// 6:41-6:57 AC
///
/// https://twitter.com/laycrs/status/1482760187127930880
///

fn solve() -> Vec<u8> {
    let mut x = readln!([u8]).iter().map(|&d| d - b'0').collect_vec();
    let n = x.len();
    if let Some(i) = (0..n - 1).rfind(|&i| x[i] + x[i + 1] > 9) {
        let sum = x[i] + x[i + 1];
        x[i] = sum / 10;
        x[i + 1] = sum % 10;
        x
