...
// $ cp-batch SubtractOrDivide | diff SubtractOrDivide.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address SubtractOrDivide.cpp && ./a.out
...
int f(int n) {
  ...
  res=1+f(n-1);
  VI ds=divisors(n);
  FORR(d,ds) SMIN(res,1+f(n/d));
  return res;
}
void test() {
  N=genRandNum(1,100);
  MINUS(memo);
  int exp=f(N),act=solve();
  dump(N,exp,act);
  assert(exp==act);
}
