...
// $ cp-batch dominant_character | diff dominant_character.out -
// $ cargo run --bin dominant_character
...
fn solve() -> i64 {
    ...
    let ts = vec!["aa", "aca", "aba", "acba", "abca", "abbacca", "accabba"];
    let ts = ts.iter().map(|&x| x.chars().collect_vec()).collect_vec();

    for t in ts {
        // dbgln!(t);
        if s.windows(t.len()).any(|cs| cs == t) {
            return t.len() as i64;
        }
    }
    return -1;
