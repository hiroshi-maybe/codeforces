...
// $ cp-batch BinarySearch | diff BinarySearch.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address BinarySearch.cpp && ./a.out
...
/*
 ...
 https://twitter.com/hanseilimak/status/1320593252752396288
...
void test() {
  ...
  vector<VV<int>> res(N+1);
  do {
    //dumpc(P);
    FORE(x,1,N) {
      if(bs(P,x)) res[x].push_back(P);
    }
  } while(next_permutation(ALL(P)));
  FORE(x,1,N) {
    dump(x);
    FORR(a,res[x]) dumpc(a);
  }
}
...
void solve() {
  ...
  int lar=N-X,sm=X-1,rem=N-cnt[0]-cnt[1]-1;
  //dump(cnt[0],cnt[1],lar,sm);
  ModInt res=com.perm(lar,cnt[1])*com.perm(sm,cnt[0])*com.fact(rem);
  cout<<res<<endl;
}
