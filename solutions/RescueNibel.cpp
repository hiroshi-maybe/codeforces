...
// $ cp-batch RescueNibel | diff RescueNibel.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address RescueNibel.cpp && ./a.out
...
/*
 ...
 https://twitter.com/hanseilimak/status/1309528973236252672
...
void solve() {
  ...
  int cur=0;
  FORR(kvp, imos) {
    int dec=kvp.second.first,inc=kvp.second.second;
    cur-=dec;
    int x=cur,y=inc;

    if(x+y>=K) {
      res+=com.choose(x+y,K);
      if(x>=K) res-=com.choose(x,K);
    }
    cur+=inc;
  }
  cout<<res<<endl;
