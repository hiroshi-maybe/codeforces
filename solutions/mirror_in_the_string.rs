...
// $ cp-batch mirror_in_the_string | diff mirror_in_the_string.out -
// $ cargo run --bin mirror_in_the_string
...
fn solve() -> Vec<char> {
    ...
    let i = (1..n).find(|&i| a[i - 1] < a[i]).unwrap_or(n);
    //dbgln!(a, i);
    let mut res = vec![];
    res.extend(&a[0..i]);
    res.extend(a[0..i].iter().rev().collect_vec());
