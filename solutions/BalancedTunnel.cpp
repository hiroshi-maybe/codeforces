...
// $ cp-batch BalancedTunnel | diff BalancedTunnel.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address BalancedTunnel.cpp && ./a.out
...
void solve() {
  VI ps(N),ps2(N);
  REP(i,N) ps[A[i]]=i;
  REP(i,N) ps2[i]=ps[B[i]];
  int res=0;
  BIT<int> bit(N);
  for(int i=N-1; i>=0; --i) {
    int cnt=bit.query(0,ps2[i]);
    bit.add(ps2[i],1);
    res+=cnt>0;
  }
  cout<<res<<endl;
