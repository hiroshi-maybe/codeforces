...
// $ cp-batch kalindrome_array | diff kalindrome_array.out -
// $ cargo run --bin kalindrome_array
...
/// 6:54-7:32 AC
///
/// https://twitter.com/laycrs/status/1463201466408001543
/// https://twitter.com/heno_code/status/1463201337219248128
/// https://twitter.com/n_vip/status/1463199614136246273
///

fn is_palin(a: &Vec<i32>) -> bool {
    a.iter().zip(a.iter().rev()).all(|(a, b)| a == b)
    /*
    let n = a.len();
    if n == 0 {
        return true;
...
fn solve() -> bool {
    ...
    if let Some((l, r)) = a.iter().zip(a.iter().rev()).find(|(a, b)| a != b) {
        for x in vec![l, r] {
            let b = a.iter().filter(|y| **y != *x).map(|y| *y).collect_vec();
            if is_palin(&b) {
                return true;
            }
        }
