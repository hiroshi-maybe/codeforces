...
// $ cp-batch reverse_sort | diff reverse_sort.out -
// $ cargo run --bin reverse_sort
...
fn solve() -> Vec<usize> {
    ...
    assert_eq!(res.len() % 2, 0);

    let mut l = 0;
    let mut r = res.len() as i32 - 1;
    while l < r {
        s.swap(res[l as usize], res[r as usize]);
        l += 1;
        r -= 1;
    }
    assert_eq!(s, ss);
