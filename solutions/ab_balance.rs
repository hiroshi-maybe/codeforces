...
// $ cp-batch ab_balance | diff ab_balance.out -
// $ cargo run --bin ab_balance
...
fn main() {
    ...
    for _ in 0..t {
        ...
        let ab = s.windows(2).filter(|s| **s == ['a', 'b']).count();
        let ba = s.windows(2).filter(|s| **s == ['b', 'a']).count();
        if ab != ba {
            assert!(s[0] != *s.last().unwrap());
            s[0] = *s.last().unwrap();
        }
