...
// $ cp-batch ToyTrainSimplified | diff ToyTrainSimplified.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address ToyTrainSimplified.cpp && ./a.out
...
void solve() {
  ...
  REP(st,N) {
    ...
    REP(i,N) if(SZ(cnt[i])>=maxcnt-1) {
      LL n=SZ(cnt[i]);
      if(n<=0) continue;
      LL t=1LL*dist(st,i)+(n-1)*N+(*cnt[i].begin());
