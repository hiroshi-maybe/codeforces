...
// $ cp-batch fun_with_even_subarrays | diff fun_with_even_subarrays.out -
// $ cargo run --bin fun_with_even_subarrays
...
fn solve() -> usize {
    ...
    let mut res = 0;
    loop {
        if let Some(cnt) = a.iter().position(|&a| a != fst) {
            res += 1;
            for i in (cnt..2 * cnt).take_while(|&i| i < n) {
                a[i] = fst;
            }
        } else {
            break;
