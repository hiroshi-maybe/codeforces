...
// $ cp-batch representative_edges | diff representative_edges.out -
// $ cargo run --bin representative_edges
...
fn solve() -> usize {
    ...
    for j in 0..n {
        for i in 0..j {
            let d = a[j] - a[i];
            let ans = (0..n)
                .filter(|&k| (a[k] - a[i]) * (j as i32 - i as i32) != (k as i32 - i as i32) * d)
                .count();

            //dbgln!(i, j, a[i], a[j], d, ans);
            res.setmin(ans);
        }
    }
